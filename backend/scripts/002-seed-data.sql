...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Mi Nombre', 'Imprime tu nombre', E'nombre = "Ana"\nprint("Hola, " + nombre)', 1, 10, 'Hola, Ana', 'output', 2
FROM modules m JOIN lessons l ON l.module_id = m.id AND l.title = 'Tu Primer Programa'
WHERE m.title = 'Introducción a Python' AND m.is_global = TRUE
ON CONFLICT DO NOTHING;
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Crear Variable', 'Crea y usa variables', E'x = 10\ny = 5\nprint(x + y)', 1, 10, '15', 'output', 1
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Tipos en acción', 'Identifica tipos', E'print(type(42))\nprint(type(3.14))\nprint(type("hola"))', 1, 10, E'<class ''int''>\n<class ''float''>\n<class ''str''>', 'output', 2
FROM modules m JOIN lessons l ON l.module_id = m.id AND l.title = 'Tipos de Datos'
WHERE m.title = 'Variables y Tipos de Datos' AND m.is_global = TRUE
ON CONFLICT DO NOTHING;

INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Operaciones', 'Operaciones básicas', E'a = 10\nb = 3\nprint(a + b)\nprint(a - b)\nprint(a * b)', 1, 10, E'13\n7\n30', 'output', 3
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Mayor de Edad', 'Condicional básico', E'edad = 18\nif edad >= 18:\n    print("Mayor de edad")\nelse:\n    print("Menor de edad")', 2, 15, 'Mayor de edad', 'output', 1
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Nota Final', 'Lógica con if/elif', E'nota = 85\nif nota >= 90:\n    print("Sobresaliente")\nelif nota >= 70:\n    print("Notable")\nelse:\n    print("Necesitas mejorar")', 2, 15, 'Notable', 'output', 2
FROM modules m JOIN lessons l ON l.module_id = m.id AND l.title = 'Condicionales'
WHERE m.title = 'Estructuras de Control' AND m.is_global = TRUE
ON CONFLICT DO NOTHING;
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Contar hasta 5', 'Bucle for básico', E'for i in range(1, 6):\n    print(i)', 2, 15, E'1\n2\n3\n4\n5', 'output', 1
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Suma Acumulativa', 'Bucle while', E'total = 0\ni = 1\nwhile i <= 5:\n    total += i\n    i += 1\nprint(total)', 2, 15, '15', 'output', 2
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Sumar dos números', 'Función con parámetros', E'def sumar(a, b):\n    return a + b\n\nprint(sumar(3, 4))', 3, 20, '7', 'output', 2
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Mi primera lista', 'Trabaja con listas', E'frutas = ["manzana", "pera", "uva"]\nprint(frutas[0])\nprint(len(frutas))', 2, 15, E'manzana\n3', 'output', 1
FROM modules m JOIN lessons l ON l.module_id = m.id AND l.title = 'Listas'
WHERE m.title = 'Listas y Tuplas' AND m.is_global = TRUE
ON CONFLICT DO NOTHING;

INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Tupla fija', 'Tuplas inmutables', E'coord = (10, 20)\nprint(coord[0])\nprint(coord[1])', 2, 15, E'10\n20', 'output', 2
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Mi diccionario', 'Usa diccionarios', E'persona = {"nombre": "Luis", "edad": 25}\nprint(persona["nombre"])', 3, 20, 'Luis', 'output', 1
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Operaciones con Sets', 'Usa conjuntos', E'a = {1, 2, 3}\nb = {3, 4, 5}\nprint(a | b)', 3, 20, '{1, 2, 3, 4, 5}', 'output', 2
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Escribir archivo', 'Escribe en un archivo', E'with open("test.txt", "w") as f:\n    f.write("Hola archivo")\nprint("Archivo creado")', 3, 20, 'Archivo creado', 'output', 1
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Usar math', 'Importa un módulo', E'import math\nprint(math.sqrt(16))', 3, 20, '4.0', 'output', 1
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Clase Robot', 'Diseña la clase Robot', E'class Robot:\n    def __init__(self, nombre):\n        self.nombre = nombre\n    def saludar(self):\n        return f"Soy {self.nombre}"\n\nr = Robot("Robo")\nprint(r.saludar())', 5, 50, 'Soy Robo', 'output', 1
...
INSERT INTO exercises (module_id, lesson_id, title, description, instructions, difficulty, points, solution_output, solution_type, "order")
SELECT m.id, l.id, 'Movimiento', 'Lógica de movimiento', E'class Robot:\n    def __init__(self):\n        self.x = 0\n    def mover(self):\n        self.x += 1\n        return self.x\n\nr = Robot()\nprint(r.mover())\nprint(r.mover())', 5, 50, E'1\n2', 'output', 2
...
SELECT c.id, '¿Qué es la IA?', 'Introducción a la Inteligencia Artificial', E'# ¿Qué es la IA?\n\nConceptos básicos de inteligencia artificial.', 1
FROM classes c WHERE c.title = 'Introducción a la IA'
ON CONFLICT DO NOTHING;

INSERT INTO class_modules (class_id, title, description, theory_content, "order")
SELECT c.id, 'Machine Learning básico', 'Primeros pasos con ML', E'# Machine Learning\n\nAprende los fundamentos del ML.', 2
FROM classes c WHERE c.title = 'Introducción a la IA'
ON CONFLICT DO NOTHING;
