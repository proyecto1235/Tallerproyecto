-- ============================================
-- Robolearn Database Schema & Stored Procedures
-- ============================================
...
CREATE TABLE IF NOT EXISTS alert_log (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT valid_alert_severity CHECK (severity IN ('low', 'medium', 'high'))
);

CREATE INDEX IF NOT EXISTS idx_alert_log_student ON alert_log(student_id);
...
BEGIN
    ...
    SELECT
        ...
        CASE WHEN (SELECT SUM(student_count) FROM module_stats) > 0
             THEN ROUND((SELECT SUM(completed_count)::NUMERIC / NULLIF(SUM(student_count), 0) * 100 FROM module_stats), 2)
             ELSE 0 END,
        (SELECT COUNT(*) FROM enrollments e
         JOIN modules m ON e.module_id = m.id WHERE m.teacher_id = p_teacher_id)::INTEGER,
        (SELECT COALESCE(json_agg(json_build_object('name', title, 'progress', ROUND(avg_pct, 2)) ORDER BY avg_pct DESC), '[]'::json)
         FROM module_stats WHERE student_count > 0);
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION fn_get_admin_stats()
RETURNS TABLE(
    total_users INTEGER, active_students INTEGER, active_teachers INTEGER,
    total_modules INTEGER, published_modules INTEGER, pending_teachers INTEGER,
    pending_reviews INTEGER, total_enrollments INTEGER, total_achievements INTEGER,
    total_exercises INTEGER
) AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION fn_get_admin_modules(
    ...
    id INTEGER, title VARCHAR, description TEXT, teacher_name VARCHAR,
    status VARCHAR, "order" INTEGER, is_published BOOLEAN, is_global BOOLEAN,
    difficulty VARCHAR, lesson_count INTEGER, exercise_count INTEGER,
    enrollment_count INTEGER, created_at TIMESTAMP
) AS $$
BEGIN
    RETURN QUERY
    SELECT m.id, m.title, m.description, u.full_name::VARCHAR,
...
CREATE OR REPLACE FUNCTION fn_get_student_alerts(
    ...
    student_id INTEGER, student_name VARCHAR, module_id INTEGER, module_title VARCHAR,
    alert_type VARCHAR, priority VARCHAR, message TEXT, progress NUMERIC,
    days_enrolled INTEGER, progress_velocity NUMERIC, avg_velocity NUMERIC
) AS $$
BEGIN
    RETURN QUERY
    ...
    student_data AS (
        SELECT e.student_id, e.module_id, e.status, e.enrolled_at,
               COALESCE(p.percentage, 0) as progress, u.full_name,
    ...
    module_avgs AS (
        SELECT module_id, AVG(progress) as avg_progress,
               AVG(progress / GREATEST(days_enrolled, 1)) as avg_velocity
        FROM student_data
        GROUP BY module_id
    )
    SELECT sd.student_id, sd.full_name::VARCHAR, sd.module_id, m.title::VARCHAR,
           CASE
...
CREATE OR REPLACE FUNCTION sp_award_points(
    p_user_id INTEGER, p_points INTEGER, p_reason VARCHAR DEFAULT NULL
) RETURNS INTEGER AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION sp_enroll_student(
    p_student_id INTEGER, p_module_id INTEGER
) RETURNS TABLE(enrollment_id INTEGER, status VARCHAR, already_enrolled BOOLEAN) AS $$
DECLARE
    v_exists INTEGER;
BEGIN
...
CREATE OR REPLACE FUNCTION fn_get_weekly_activity(
    p_teacher_id INTEGER
) RETURNS TABLE(day_name VARCHAR, active_count INTEGER) AS $$
BEGIN
    RETURN QUERY
    SELECT
...
BEGIN
    ...
    WHERE
        (e.criteria->>'type' = 'exercise_complete'
         AND (SELECT COUNT(*) FROM exercise_attempts WHERE student_id = p_user_id AND passed = TRUE) >= COALESCE((e.criteria->>'count')::INTEGER, 1))
        OR
        ...
        (e.criteria->>'type' = 'challenge_complete'
         AND (SELECT COUNT(*) FROM challenge_attempts WHERE student_id = p_user_id AND passed = TRUE) >= COALESCE((e.criteria->>'count')::INTEGER, 1))
        OR
        (e.criteria->>'type' = 'first_try'
         AND (SELECT COUNT(*) FROM (SELECT exercise_id FROM exercise_attempts WHERE student_id = p_user_id AND passed = TRUE AND attempt_count = 1 GROUP BY exercise_id) sub) >= COALESCE((e.criteria->>'count')::INTEGER, 1));
    ...
    FROM (
        ...
        AND (
            (a.criteria->>'type' = 'exercise_complete'
             AND (SELECT COUNT(*) FROM exercise_attempts WHERE student_id = p_user_id AND passed = TRUE) >= COALESCE((a.criteria->>'count')::INTEGER, 1))
            OR
...
CREATE OR REPLACE FUNCTION fn_get_student_performance_distribution(
    p_teacher_id INTEGER
) RETURNS TABLE(category VARCHAR, value INTEGER) AS $$
BEGIN
    RETURN QUERY
    WITH student_progress AS (
        SELECT p.student_id, AVG(p.percentage) as avg_pct
        FROM progress p
...
CREATE OR REPLACE FUNCTION fn_get_module_detail_for_admin(
    ...
    module_id INTEGER, title VARCHAR, description TEXT, theory_content TEXT,
    teacher_id INTEGER, teacher_name VARCHAR, status VARCHAR, "order" INTEGER,
    is_published BOOLEAN, is_global BOOLEAN, difficulty VARCHAR, lesson_count INTEGER,
    created_at TIMESTAMP, lessons JSON, exercises JSON
) AS $$
BEGIN
    RETURN QUERY
    SELECT m.id, m.title, m.description, m.theory_content,
           ...
           (SELECT COALESCE(json_agg(json_build_object('id', l.id, 'title', l.title, 'theory_content', l.theory_content, 'order', l."order") ORDER BY l."order"), '[]'::json) FROM lessons l WHERE l.module_id = m.id),
           (SELECT COALESCE(json_agg(json_build_object('id', e.id, 'title', e.title, 'description', e.description, 'instructions', e.instructions, 'difficulty', e.difficulty, 'points', e.points, 'order', e."order", 'solution_output', e.solution_output, 'solution_type', e.solution_type) ORDER BY e."order"), '[]'::json) FROM exercises e WHERE e.module_id = m.id)
    FROM modules m
    LEFT JOIN users u ON m.teacher_id = u.id
...
CREATE OR REPLACE FUNCTION fn_search_users(
    p_search VARCHAR DEFAULT NULL, p_role VARCHAR DEFAULT NULL,
    p_limit INTEGER DEFAULT 50, p_offset INTEGER DEFAULT 0
) RETURNS TABLE(
    id INTEGER, email VARCHAR, full_name VARCHAR, role VARCHAR,
    is_active BOOLEAN, points INTEGER, streak_days INTEGER, created_at TIMESTAMP
) AS $$
BEGIN
    RETURN QUERY
...
DO $$
BEGIN
    RAISE NOTICE 'Robolearn schema and procedures created successfully';
END $$;
